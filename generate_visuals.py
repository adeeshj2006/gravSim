...
# Save screenshot (first frame)
# To get a specific frame, we could set the frame number, but we can just save the current figure
# after drawing the first frame by calling update(0) manually? Simpler: we save the figure after
# the animation is built but before saving? Actually the figure is empty until we draw.
# We'll just call update(0) to draw first frame then save.
update(0)
plt.savefig('./screenshots/simulation_screenshot.png', dpi=150)
print("Screenshot saved.")

# Save animation as GIF
writer = PillowWriter(fps=20)
ani.save('./gifs/orbital_motion.gif', writer=writer)
print("GIF saved.")